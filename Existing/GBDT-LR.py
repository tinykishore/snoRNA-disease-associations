...
file_known_dma = open(r'.\data\Known disease-miRNA association number.xlsx')
file_dss1 = open(r'.\data\Disease semantic similarity matrix 1.xlsx')
file_dss2 = open(r'.\data\Disease semantic similarity matrix 2.xlsx')
file_dssw = open(r'.\data\Disease semantic similarity weighting matrix.xlsx')
file_mfs = open(r'.\data\miRNA functional similarity matrix.xlsx')
file_mfsw = open(r'.\data\miRNA functional similarity weighting matrix.xlsx')
file_mn = open(r'.\data\miRNA number.xlsx')
file_dn = open(r'.\data\disease number.xlsx')
...
for i in range(383):
    for j in range(383):
        ...
        m = s1[j]
        n = s2[j]
        SS[i, j] = float(m + n) / 2  # 综合疾病语义相似性 Obtain disease semantic similarity SS
...
gamd = 383 / (LA.norm(C,
                      'fro') ** 2);  # norm(X,'fro') 计算矩阵 (向量) C的F范数;F范数是把一个矩阵中每个元素的平方求和后开根号；**两个乘号就是乘方，比如2**4,结果就是2的4次方，结果是16
...
file_mfsw.writelines(['disease', '\t', 'miRNA', '\t', 'Score', '\n'])
file_mfsw.close()
